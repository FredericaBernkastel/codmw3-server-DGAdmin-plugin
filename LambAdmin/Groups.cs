            ...
            public GroupsDatabase()
            {
                ...
                if (!File.Exists(ConfigValues.ConfigPath + @"Groups\groups.txt"))
                {
                    ...
                    File.WriteAllLines(ConfigValues.ConfigPath + @"Groups\groups.txt", new string[]
                    {
                        ...
                        "admin:DGAdmin:scream,receiveamsg,whois,changeteam,yell,gametype,mode,login,map,status,unban,kick,tmpban,ban,warn,unwarn,getwarns,res,setafk,setteam,balance,clanvsall,clanvsallspectate,login,apply,night:^0[^1Admin^0]^7",
